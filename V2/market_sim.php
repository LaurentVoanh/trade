<?php
// market_sim.php - Simulation du marché CryptoFun V2
// À exécuter via cron toutes les 5-15 min
require 'config.php';
...
$botUsers = $db->query("SELECT id FROM users WHERE id>1 LIMIT 50")->fetchAll(PDO::FETCH_COLUMN);
require 'bot_engine.php';
$engine = new BotEngine();

foreach($botUsers as $uid) {
    if(rand(0,100) < 30) {
        $engine->runPerformanceBot($uid);
    }
}
